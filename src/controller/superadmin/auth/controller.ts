...
const SUPERADMIN_ACCESS_TOKEN_SECRET =
...
export function signSuperAdminAccessToken(
  ...
  return jwt.sign(payload, SUPERADMIN_ACCESS_TOKEN_SECRET, {
...
export function generateSuperAdminRefreshToken() {
  ...
  const tokenHash = crypto
    .createHmac("sha256", process.env.SUPERADMIN_REFRESH_TOKEN_SECRET!)
    .update(token)
    .digest("hex");
