...
export type OrgListRow = { id: string; name: string; slug: string; status: string; createdAt: Date };
...
// Platform-wide, not scoped by org - a superadmin sees every org that
// exists. Deliberately lean columns only, no joins to patients/locations
// counts, keeping this list query as fast as possible.
...
export type UpdateOrgResult =
  | { success: true; organization: { id: string; name: string; slug: string; photoUrl: string | null; status: string } }
  | { success: false; error: string; code: OrgErrorCode };
...
export async function updateOrganization(orgId: string, input: unknown): Promise<UpdateOrgResult> {
  try {
    ...
    return {
      success: true,
      organization: { ...updated, photoUrl: imagePresets.thumbnail(updated.photoUrl) },
    };
  } catch (err) {
    if (err instanceof SuperAdminSessionError) {
      return { success: false, error: err.message, code: "UNAUTHORIZED" };
