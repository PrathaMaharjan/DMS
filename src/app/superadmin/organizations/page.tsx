...
export default function OrganizationsPage() {
  ...
  function toggleStatus(o: Organization) {
    const nextStatus: OrgStatus = o.status === "Active" ? "Inactive" : "Active";
    setOrgs((prev) =>
      prev.map((x) => (x.id === o.id ? { ...x, status: nextStatus } : x))
    );
    setSelectedOrg((prev) =>
      prev && prev.id === o.id ? { ...prev, status: nextStatus } : prev
    );
  }
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 px-4">
          ...
          <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-2xl">
            ...
            <p className="mt-1.5 text-[0.85rem] leading-relaxed text-slate-500">
              This deletes the organization and all of its outlets, staff, and data. This can't be undone.
            </p>
