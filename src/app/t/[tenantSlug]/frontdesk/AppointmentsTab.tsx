...
export default function AppointmentsTab() {
  ...
  const [appointments, setAppointments] = useState([
    { id: "1", patient: "Aayush Shrestha", phone: "9841234567", email: "aayush@gmail.com", dentist: "Pratha Maharjan", service: "Routine Checkup", time: "09:00", source: "Online", status: "Checked In" },
  ...
  // Quick Register State (Fields matching the core booking expectations)
  const [showQuickRegister, setShowQuickRegister] = useState(false);
  const [registerForm, setRegisterForm] = useState({ name: "", phone: "", email: "", insurance: "Self Pay" });

  // Filtered list of registered patients for the walkthrough selection
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {filteredAppointments.map((appt) => (
                <tr key={appt.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4"><span className="inline-flex items-center gap-1 text-xs text-slate-700 font-medium"><Clock className="h-3 w-3 text-sky-400" /> {appt.time}</span></td>
                  <td className="p-4">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      appt.status === "Checked In" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" :
                      appt.status === "No-Show" ? "bg-rose-50 text-rose-700 border border-rose-200" : "bg-sky-50 text-sky-700 border border-sky-100"
                    }`}>{appt.status}</span>
                  </td>
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-1.5">
                      <button onClick={() => setAppointments(appointments.map(a => a.id === appt.id ? {...a, status: "Checked In"} : a))} className="rounded-lg p-1 text-emerald-600 hover:bg-emerald-50"><Check className="h-4 w-4" /></button>
                      <button onClick={() => setAppointments(appointments.map(a => a.id === appt.id ? {...a, status: "No-Show"} : a))} className="rounded-lg p-1 text-rose-600 hover:bg-rose-50"><X className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
