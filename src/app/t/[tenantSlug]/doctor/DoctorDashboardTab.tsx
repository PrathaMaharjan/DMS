    ...
    const loadData = useCallback(async () => {
        try {
            ...
            if (res?.data?.success && res.data.data?.dashboard) {
                ...
                setErrorMsg(res?.data?.error || "Failed to load doctor dashboard data.");
            }
        } catch (err: any) {
                    ...
                    <>
                        ...
                        <div className="grid gap-4 lg:grid-cols-3">
                            ...
                            <div className="rounded-2xl border border-[#7da3b3]/20 bg-gradient-to-br from-[#7da3b3]/10 via-white to-white p-6 shadow-lg backdrop-blur-sm">
                                ...
                                {upNext ? (
                                    <div>
                                        <p className="text-lg font-bold text-slate-900">{upNext.patientName}</p>
                                        <p className="mt-1 flex items-center gap-1.5 text-xs text-slate-500">
                                            <Clock className="h-3.5 w-3.5 text-[#7da3b3]" /> {upNext.startTime}
                                            <span className="text-slate-300">·</span>
                                            <Stethoscope className="h-3.5 w-3.5 text-[#7da3b3]" /> {upNext.treatmentName}
                                        </p>
                                        {upNext.patientPhone && (
                                            <p className="mt-1 flex items-center gap-1.5 text-xs text-slate-500">
                                                <Phone className="h-3.5 w-3.5 text-slate-400" /> {upNext.patientPhone}
                                            </p>
                                        )}
                                        {upNext.notes ? (
                                            ...
                                            return (
                                                <div key={a.id} className="flex items-center gap-3 p-4">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <p className="text-xs text-slate-500 truncate">{a.treatmentName}</p>
                                                    </div>
                                        ...
                                        recentPatients.map((p) => (
                                            ...
                                            >
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <p className="text-xs text-slate-500 truncate">{p.treatmentName}</p>
                                                </div>
                                                <span className="shrink-0 text-xs text-slate-400">{p.date}</span>
                                            </button>
                                        ))
                                    )}
                                </div>
